...
3. `TileCache` holds the results — thread-safe LRU, 128 MB budget, ref-counted `IRef<TileImage>` so a tile cannot be freed while a render pass still draws it. There is **one cache per open document** (`PdfPageService` is DI-scoped, and `PdfDocumentsManagerService` creates one scope per document), so host-side tile memory scales with the number of open documents. 96 MB was chosen empirically: a realistic reading-session workload (forward scroll, a near-range back-and-forth revisit, a 100%-600% zoom cycle, more forward scroll) measured a zero-wasted-re-render floor of 48 MB and a worst-case peak of 72 MB — the LRU evicts precisely enough that budgets well below the true peak still cost no extra re-renders, so 96 MB carries a ~33% margin over the measured peak, not a razor-thin one. Larger pages, bigger monitors, or heavier zoom/scroll than what was tested would eat into that margin faster than the previous 128 MB default would have.
